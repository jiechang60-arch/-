...
(function () {
  ...
  R.monk = function (ctx, x, y, s, hearts, maxHearts, shake, t, pokeT) {
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('斗', 3, s * 0.05 + 3);
    // 斗字主体（深棕色，加粗）
    ctx.fillStyle = '#3a2a1a';
    ctx.fillText('斗', 0, s * 0.02);
    ctx.restore();
    // 金箍（替代金冠）
  ...
  // id 为人物标识（zhaoyun/machao/guanyu/zhangfei/huangzhong/liubei）
  var avatarPalette = {
    zhaoyun:    { bg: '#c8b46a', face: '#f4d9a8', accent: '#a8402c' },
    machao:     { bg: '#6a9ad0', face: '#f0dcb8', accent: '#2a4a6a' },
    guanyu:     { bg: '#a85e3a', face: '#e8c0a0', accent: '#3a7a4a' },
    zhangfei:   { bg: '#4a5a6a', face: '#e8c0a0', accent: '#1a2a3a' },
    huangzhong: { bg: '#8a8a5a', face: '#f0dcb8', accent: '#c8c8d0' },
    liubei:     { bg: '#5a8a6a', face: '#f4d9a8', accent: '#e8c53a' }
  };
  R.avatar = function (ctx, x, y, r, id, selected) {
    ctx.save();
    var pal = avatarPalette[id] || avatarPalette.zhaoyun;
    // 圆形彩色背景
    ...
    if (id === 'zhaoyun') {
      ...
      ctx.beginPath();
      ctx.arc(x, y - r * 0.3, r * 0.55, Math.PI, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = '#a8402c';
      ctx.beginPath();
      ctx.ellipse(x, y - r * 0.78, r * 0.14, r * 0.3, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = '#8a6a10';
      ctx.lineWidth = r * 0.06;
      ctx.beginPath();
      ...
      ctx.fillStyle = pal.bg;
      ctx.beginPath();
      ctx.arc(x, y - r * 0.32, r * 0.58, Math.PI, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = '#e8edf5';
      for (var m1 = -2; m1 <= 2; m1++) {
        ctx.beginPath();
        ctx.arc(x + m1 * r * 0.3, y - r * 0.34, r * 0.14, 0, Math.PI * 2);
        ctx.fill();
      }
    } else if (id === 'guanyu') {
      // 绿帽 + 长髯
      ...
      ctx.fillStyle = '#2a2a2a';
      ctx.beginPath();
      ctx.moveTo(x - r * 0.22, y + r * 0.18);
      ctx.quadraticCurveTo(x, y + r * 0.95, x + r * 0.22, y + r * 0.18);
      ctx.closePath();
      ctx.fill();
      // 卧蚕眉
      ...
      ctx.quadraticCurveTo(x - r * 0.2, y - r * 0.38, x - r * 0.05, y - r * 0.24);
      ctx.moveTo(x + r * 0.45, y - r * 0.22);
      ctx.quadraticCurveTo(x + r * 0.2, y - r * 0.38, x + r * 0.05, y - r * 0.24);
      ctx.stroke();
    } else if (id === 'zhangfei') {
      ...
      ctx.beginPath();
      ctx.ellipse(x, y + r * 0.38, r * 0.42, r * 0.3, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.ellipse(x, y - r * 0.32, r * 0.5, r * 0.22, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = '#1a1a1a';
      ctx.lineWidth = r * 0.06;
      ctx.beginPath();
      ctx.arc(x - r * 0.2, y - r * 0.05, r * 0.15, 0, Math.PI * 2); ctx.stroke();
      ctx.beginPath(); ctx.arc(x + r * 0.2, y - r * 0.05, r * 0.15, 0, Math.PI * 2); ctx.stroke();
    } else if (id === 'huangzhong') {
      // 白发白须老将
      ctx.fillStyle = '#e8e8e8';
      ctx.beginPath();
      ctx.arc(x, y - r * 0.3, r * 0.5, Math.PI, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(x - r * 0.2, y + r * 0.2);
      ctx.quadraticCurveTo(x, y + r * 0.85, x + r * 0.2, y + r * 0.2);
      ctx.closePath();
      ctx.fill();
      // 皱纹
      ...
      ctx.fillStyle = pal.bg;
      ctx.beginPath();
      ctx.arc(x, y - r * 0.38, r * 0.5, Math.PI, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = '#e8c53a';
      ctx.beginPath();
      ctx.arc(x, y - r * 0.55, r * 0.08, 0, Math.PI * 2);
      ctx.fill();
