...
(function () {
  ...
  B.tickFarmers = function (S, side, dt) {
    if (side === 'e' && ZY.G && ZY.G.mode === 'pvp') return;
    for (var k in S.units) {
      var u = S.units[k];
      if (u.kind !== 'farm') continue;
  ...
  // 随机源走 ZY.Rng.rand：单机=Math.random，联机=对局种子（双方手牌一致）
