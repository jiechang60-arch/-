// 地图：8列×10行，上半场为对手，下半场为玩家，中央山脊分隔
// 正式版风格的四张地图布局；每张图玩家侧 S 形土路 + 镜像对手侧
(function () {
  ...
  // 选定本局地图：优先用种子随机（联机双方一致），否则 Math.random
  function pickRandom() {
    var idx;
    if (ZY.Rng && ZY.Rng.curSeed != null) {
      var rnd = ZY.Rng.derive(ZY.Rng.curSeed ^ 0x5EED);
      idx = Math.floor(rnd() * LAYOUTS.length);
    } else {
  ...
  M.pickIndex = pickIndex;       // 指定地图（联机用）
  M.curIndex = function () { return curIndex; };
